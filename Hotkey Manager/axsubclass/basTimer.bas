...
Const clsTimerMax = 100

' Array of timers
Public aTimers(1 To clsTimerMax) As clsTimer
' Added SPM to prevent excessive searching through aTimers array:
Private m_clsTimerCount As Integer
...
Function TimerCreate(timer As clsTimer) As Boolean
    ...
    If timer.TimerID Then
        TimerCreate = True
        Dim i As Integer
        For i = 1 To clsTimerMax
            If aTimers(i) Is Nothing Then
                Set aTimers(i) = timer
                If (i > m_clsTimerCount) Then
                    m_clsTimerCount = i
                End If
                TimerCreate = True
                Exit Function
