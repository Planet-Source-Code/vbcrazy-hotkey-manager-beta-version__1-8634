VERSION 5.00
Begin VB.UserControl AxGridCtl 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
...
Dim iRow As Long
   ...
   If (lRow > 0) And (lRow <= m_iRows) Then
      ...
      End If
   Else
      Err.Raise 9, App.EXEName & ".AxGridCtl"
   End If
End Property
   ...
   If (sKey <> "") Then
      For i = 1 To m_iCols
         If (m_tCols(i).sKey = sKey) Then
            Err.Raise 457, App.EXEName & ".AxGridCtl"
            Exit Function
         End If
      Next i
   End If
...
Private Function pbValid(ByVal lRow As Long, ByVal lCol As Long) As Boolean
   If (lCol > 0) And (lCol <= m_iCols) Then
      If (lRow > 0) And (lRow <= m_iRows) Then
         pbValid = True
      Else
         Err.Raise 9, App.EXEName & ".AxGridCtl", "Invalid Row Index"
      End If
   Else
      Err.Raise 9, App.EXEName & ".AxGridCtl", "Invalid Column Index"
   End If
End Function
...
Dim lPreviousStartY As Long
   ...
   If (lRow > 0) Then
      ...
      End If
   Else
      Err.Raise 9, App.EXEName & ".AxGridCtl", "Row subscript out of range"
   End If
End Property
...
Dim iVisibleCols As Long
   ...
   Else
      Err.Raise 9, App.EXEName & ".AxGridCtl", "Column subscript out of range"
   End If
...
Dim iCol As Long
   If (lRows > 0) Then
      If (m_iCols = 0) Then
         Err.Raise 9, App.EXEName & ".AxGridCtl", "Attempt to add rows with no columns."
      Else
